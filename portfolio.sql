...
CREATE TABLE portfolio (
    ...
    u_id INTEGER,
    CONSTRAINT FK_UserPortfolio FOREIGN KEY(u_id) REFERENCES user(u_id)
);
...
CREATE TABLE portfolio_stock (
    ...
    p_id INTEGER,
    CONSTRAINT FK_PortfolioStock FOREIGN KEY(p_id) REFERENCES portfolio(p_id),
...
CREATE TABLE portfolio_stock_transaction (
    ...
    ps_id INTEGER,
    CONSTRAINT FK_PortfolioStockTransaction FOREIGN KEY(ps_id) REFERENCES portfolio_stock(ps_id)
);
...
CREATE TABLE watchlist (
    ...
    u_id INTEGER,
    CONSTRAINT FK_UserWatchlist FOREIGN KEY(u_id) REFERENCES user(u_id)
);
...
CREATE TABLE watchlist_stock (
    ...
    symbol VARCHAR(64) NOT NULL,
    w_id INTEGER,
    CONSTRAINT FK_WatchlistStock FOREIGN KEY(w_id) REFERENCES watchlist(w_id),
